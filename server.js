...
app.get('/student', async (req, res) => {
    ...
    searchfname = await queryExecutor(`SELECT * FROM exam_admin.exam_master where 
    exam_master.exam_name='${str}'`);
    if (!searchfname) {
        searchfname = await queryExecutor(`select * from exam_master where exam_name like '%${str}%'`);
    }
    if (str.length == "") {
        searchfname = await queryExecutor(`SELECT * FROM exam_admin.exam_master;`);
    }
